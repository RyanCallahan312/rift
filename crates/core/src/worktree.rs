...
pub(crate) fn initialize_root(
    ...
    ensure_inline_git_dir(&inline_git)?;
...
pub(crate) fn validate_init_root(path: &Path) -> Result<()> {
    ensure_inline_git_dir(&path.join(".git"))?;
    ensure_toplevel(path)?;
    ensure_head_commit(path)
}
...
fn ensure_inline_git_dir(inline_git: &Path) -> Result<()> {
...
fn copy_git_directory(from: &Path, to: &Path) -> Result<()> {
    fs::create_dir(to)?;
